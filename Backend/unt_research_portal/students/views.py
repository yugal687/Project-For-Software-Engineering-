...
from rest_framework import status
from .models import Student
from professor.models import ResearchOpportunity, StudentApplication
...
class ApplyResearchOpportunityView(APIView):
    def post(self, request):
        ...
        if score < threshold:
            application.delete()  # Cleanup invalid applications
            return Response({"score": f"{score:.2f}","Message": f"Resume score too low ({score:.2f}%). Minimum required: {threshold}%."}, status=status.HTTP_201_CREATED)
